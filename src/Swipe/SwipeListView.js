...
class SwipeListView extends Component {
	...
	switchLeftRadio( callback ){
		this.openCellKey=null;
		if(!this.isCheckBoxShow){
			this.openLeftRadio(callback);
		}else{
			this.closeLeftRadio(callback);
		}
	}

	closeLeftRadio(callback){
		Object.keys(this._rows).forEach((key)=>{
			this._rows[key].openLeft(0, false);
		});
		this.isCheckBoxShow = false;
		callback && callback(this.isCheckBoxShow);
	}

	openLeftRadio(callback){
		Object.keys(this._rows).forEach((key)=>{
			this._rows[key].openLeft(65, true);
		});
	...
	renderCell(VisibleComponent, HiddenComponent, key, item, rowData, onItemTap, shouldPreviewRow) {
		if (!HiddenComponent) {
			return React.cloneElement(
				VisibleComponent,
				{
					...
					onRowPress: () => this.onRowPress(rowData, onItemTap),
	...
	renderRow(rowData, onItemTap, secId, rowId, rowMap) {
		...
		return this.renderCell(Component, HiddenComponent, key, rowData, onItemTap, shouldPreviewRow);
	}

	renderItem(rowData, onItemTap, rowMap) {
		...
		return this.renderCell(Component, HiddenComponent, key, item, rowData, onItemTap, shouldPreviewRow);
	}

	render() {
		const { useFlatList, useSectionList, onItemTap, renderListView, ...props } = this.props;
		...
		if (useFlatList) {
			return (
				<FlatList
					...
					renderItem={(rowData) => this.renderItem(rowData, onItemTap, this._rows)}
				/>
			);
		}
		...
		return (
			<ListView
				...
				ref={ c => this.setRefs(c) }
				onScroll={ e => this.onScroll(e) }
				renderRow={(rowData, secId, rowId) => this.renderRow(rowData, onItemTap, secId, rowId, this._rows)}
			/>
		);
	}
